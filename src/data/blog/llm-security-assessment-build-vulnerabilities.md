---
title: "LLM 보안 진단에서 취약점을 빌드한다는 것"
pubDatetime: 2026-05-09T15:57:43+09:00
tags: ["LLM", "AI Security", "prompt-injection", "security-assessment", "checklist"]
description: "엑셀 업무 보조에 붙은 LLM 보안 진단 경험을 일반화해, LLM 취약점 판단 기준과 위협 시나리오를 어떻게 세워야 하는지 정리한 회고록"
---

## 개요

이번 LLM 취약점 진단에서 가장 힘들었던 것은 취약점을 찾는 일 자체보다, 발견한 현상이 정말 취약점인지 끝까지 고민해 보는 일이었다. 내가 제출한 취약점에 대해 보안 조치를 할 개발팀이 과연 이 취약점을 납득하고 조치할까? 라는 질문을 스스로에게 던졌는데, 던질 때마다 의문이 끊임없이 들었기 때문이다.

그리고...

1. 주변에 LLM 취약점을 진단해 본 사람이 없어서 자문을 구할 방법이 없었다. 거의 인터넷 검색과 ChatGPT에만 의존했다. 게다가 GPT도 믿을 수 없는 놈이다.
2. 전통적인 웹 취약점은 상대적으로 증거가 선명하다. SQL Injection은 쿼리가 실행되고, XSS는 스크립트가 실행되고, 파일 업로드 취약점은 웹 셸이 업로드되어 명령어 실행 같은 결과가 나온다.
...
LLM은 그냥 ChatGPT, Claude Code... 내가 아는 건 이게 전부였다. 그런 나한테 LLM 취약점 진단이 떨어졌다.
...
| 모델이 실행 중인 서버에 접근할 수 있는가? | 모델 및 에이전트 항목 체크리스트 기반 진단이 가능해진다. |
...
오정보 생성 취약점에서 가장 중요한 포인트는 **오정보 생성으로 인해 사용자가 *행동*까지 할 수 있는가**이다.
